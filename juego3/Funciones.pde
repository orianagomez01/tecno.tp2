...
void agregarLluvia() {
  ...
  for (int i=0; i<ballCount; i++) {
    ...
    bolita.setDensity(6000);
    bolita.addForce(6000, bf);
...
//void crearFuego () {
    //for ( int i= 0; i < 10; i ++) {
    //   rect (150*i, height-200, 250, 200); 
    // }
  

  //void aplicarFuerza(FBox cajas, FBox bolitas) {
  //  cajas = mundo.getBodies();

  //}
//}

void temporizador () {
   //float s = map (second(), 0, 60, width-30, height/36);

//arreglar para que quede el rect antes de q termine la pantalla  
 float s = map (second(), 0, 60, height/36, width);
  
  noStroke();
  //cambiar el color
fill(s/3, 0, s/3);
rect (width/2+350, 20, s, height/24);
//stroke (255);
//line (s, 0, s, height/24);
 
 //if ( s >= 0 ) {
 //resetear  
 //colision de que perdió
 //}
